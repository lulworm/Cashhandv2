...
TopBar::TopBar(CASHHANDGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopCHND}, "amount-small-topbar");
    setCssProperty({ui->labelAmountCHND}, "amount-topbar");
    setCssProperty({ui->labelPendingCHND, ui->labelLockedCHND}, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& lockedBalance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    /* CHND Total */
    // CASHHAND excludes "locked" CHND from the Available balance to improve UX
    CAmount chndAvailableBalance = balance - lockedBalance;
    QString totalCHND = GUIUtil::formatBalance(chndAvailableBalance, nDisplayUnit);

    /* CHND Available Balance */
    // Top
    ui->labelAmountTopCHND->setText(totalCHND);
    // Expanded
    ui->labelAmountCHND->setText(totalCHND);

    /* CASHHAND merged "Pending" and "Immature" into a single GUI balance, to simplify the experience for the user */
    // Locked
    ui->labelLockedCHND->setText(GUIUtil::formatBalance(lockedBalance, nDisplayUnit));
    // Pending + Immature
    ui->labelPendingCHND->setText(GUIUtil::formatBalance((unconfirmedBalance + immatureBalance), nDisplayUnit));
}
