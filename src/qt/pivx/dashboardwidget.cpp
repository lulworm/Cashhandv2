...
DashboardWidget::DashboardWidget(CASHHANDGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    ui->labelMessage->setText(tr("Amount of CHND earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareCHND, "square-chart-chnd");
    setCssProperty(ui->labelSquareMNRewards, "square-chart-mnrewards");
    setCssProperty(ui->labelCHND, "text-chart-chnd");
    ...
    ui->labelAmountMNRewards->setText("0 CHND");
    ui->labelAmountCHND->setText("0 CHND");
    setCssProperty(ui->labelAmountCHND, "text-stake-chnd-disable");
    ...
    ui->btnHowTo->setText(tr("How to get CHND"));
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair CHND and MN rewards
...
void DashboardWidget::onChartRefreshed() {
    ...
    if (chartData->totalStakes > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountCHND, "text-stake-chnd");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards");
    } else {
        setCssProperty(ui->labelAmountCHND, "text-stake-chnd-disable");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards-disable");
    }
    forceUpdateStyle({ui->labelAmountCHND, ui->labelAmountMNRewards});
    ui->labelAmountCHND->setText(GUIUtil::formatBalance(chartData->totalStakes, nDisplayUnit) + " (Stakes)");
